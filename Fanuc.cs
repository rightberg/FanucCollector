...
namespace OpcUaDeviceControlServer
{
	public class FanucCollector()
	{
		public string mode { get; set; }
		public string run_state { get; set; }
		public string status { get; set; }
		public string shutdowns { get; set; }
		public string hight_speed { get; set; }
		
		public int prog_num { get; set; }
		public int sub_prog_num { get; set; }
    ...
    public class Fanuc
    {
		public FanucCollector GetCollector(string id, ushort port, int timeout, string series="0i-F")
		{
			FanucCollector collector = new();
			ushort handle;
			short result;
			Focas1.ODBST buf = new Focas1.ODBST();
			result = Focas1.cnc_allclibhndl3(id, port, timeout, out handle);
			if (result != Focas1.EW_OK)
			{
				collector.mode = GetMode(handle, series);
				collector.run_state = GetRunState(handle, series);
				collector.status = GetStatus(handle, series);
				collector.shutdowns = GetShutdowns(handle);
				collector.hight_speed = GetHightSpeed(handle);

				collector.prog_num = GetMainProgramNumber(handle);
				collector.sub_prog_num = GetSubProgramNumber(handle);
				collector.prog_name = GetMainProgramName(handle);
				collector.sub_name = GetSubProgramName(handle);
			}
			return collector;
		}

		private string GetCncErrorMessage(short error_code)
		{
		...
		public string GetMainProgramName(ushort handle)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
			else
		...
		public double AbsolutePosition(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public double RelativePosition(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public double MachinePosition(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public double GetAllAxisAbsolutePositions(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public string GetModeString(short mode, string series)
		{
		...
		public string GetMode(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public string GetRunStateString(short mode, string series)
		{
			if (series == "0i-D" || series == "0i-F")
			{
				return mode switch
		...
		public string GetRunState(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public string GetStatusString(short status)
		{
		...
		public string GetStatus(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public string GetShutdowns(ushort handle)
		{
		...
		public string GetHightSpeed(ushort handle)
		{
			byte[] buf = new byte[256];
			short block_count = 0;
			ushort block_length = (ushort)buf.Length;
		...
		private string GetAxisMotionString(int motion, string series)
		{
			if (series == "0i-D" || series == "0i-F")
			{
		...
		public string GetAxisMotion(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		private string GetAxisMstbString(int motion, string series)
		{
			if (series == "0i-D" || series == "0i-F")
			{
				return motion switch
		...
		public string GetMstb(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		private string GetLoadExcessString(int motion, string series)
		{
			if (series == "0i-D" || series == "0i-F")
			{
				return motion switch
		...
		public string GetLoadExcess(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		*/
		...
		public int GetMainProgramNumber(ushort handle)
		{
			if (handle == 0)
				return 0;
			else
		...
		public int GetSubProgramNumber(ushort handle)
		{
			if (handle == 0)
				return 0;
			else
		...
		public string GetFrame(ushort handle)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
			else
		...
		public int GetPartsCount(ushort handle)
		{
			if (handle == 0)
				return 0;
			else
		...
		public int GetToolNumber(ushort handle)
		{
			if (handle == 0)
				return 0;
			else
		...
		public int GetFrameNum(ushort handle)
		{
			if (handle == 0)
				return 0;
			else
		...
		private string GetUnitString(short unit)
		{
			return unit switch
			{
				0 => "mm/min",
		...
		public string GetFeedUnit(ushort handle)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public string GetSpindleUnit(ushort handle)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public int GetFeedRate(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		private int GetFeedOverrideValue(short fov)
		{
			return fov switch
			{
				0 => 0,
		...
		public int GetFeedOverride(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		private float GetJogOverrideValue(short fov)
		{
			return fov switch
			{
				0 => 0,
		...
		public float GetJogOverride(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public int GetJogSpeed(ushort handle)
		{
		...
		public Dictionary<string, int> GetAllServoLoad(ushort handle)
		{
		...
		public float GetServoCurrentLoad(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		// CNC_RDCURRENT
		public float GetServoCurrentPercentLoad(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public int GetSpindleSpeed(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public string GetSpindleSpeedParameter(ushort handle)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
		...
		public int GetMotorSpeedSpindle(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public int GetSpindleLoad(ushort handle)
		{
			if (handle == 0)
				return 0;
		...
		public int GetSpindleOveeride(ushort handle)
		{
			return 100;
		}
		#endregion
		...
		private string GetAlarmString(int alarm, string series)
		{
			if (series == "0i-D" || series == "0i-F")
			{
				return alarm switch
		...
		public string GetAlarm(ushort handle, string series)
		{
			if (handle == 0)
				return "НЕТ ДОСТУПА";
